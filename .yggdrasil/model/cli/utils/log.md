...
## [2026-06-12T12:30:57.442Z]
The aspect when: parser now produces a cross-hint error when a path or content atom appears in aspect when: context. These are file-predicate atoms that belong in scope.files — the error message retains the original 'unknown when operator' prefix (so existing callers that match on that pattern continue to work) and appends the cross-hint explanation. Similarly, the file-when parser now produces a cross-hint error when a node, relations, or descendants atom appears in scope.files context, with a message that retains the 'unknown when key' prefix and appends the cross-hint. This separates the two filter families at the parse layer with actionable guidance rather than a generic unknown-key error.
