...
const GITIGNORE_LINES = ['yg-secrets.yaml', '.symbols-cache/', '.ast-cache/', '.debug.log', '.yg-lock.deterministic.json'];
...
describe('ensureYggdrasilGitignore', () => {
  ...
  it('appends only the missing line, preserving other content (idempotent)', async () => {
    ...
    const gi = await readFile(path.join(yggRoot, '.gitignore'), 'utf-8');
    // The missing lines (.ast-cache/, .debug.log, .yg-lock.deterministic.json) were appended once; existing content preserved.
