...
- **Documentation for relation conformance across the rules / knowledge / user-docs surface.** The agent rules primer (`rules.ts`, regenerating `agent-rules.md`) now describes the built-in relation-conformance check — always-error, not an aspect, not status-governed, not suppressible, one-directional, mapped-target-only — and how to fix a refusal (declare the relation or remove the dependency). The `ports-and-relations`, `verification-and-lock` (lock format v2, the `relation_verdicts` section, parse-free re-validation, and the v1→v2 migration), `cli-reference` (the `relation-undeclared-dependency` issue code), and `suppress-syntax` (the check is not suppressible) knowledge topics gained matching sections, and the user docs (`core-concepts`, `cli-reference`) describe the check.
