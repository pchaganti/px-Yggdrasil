...
Verification runs per `(aspect, unit)` pair. A **unit** is the subject of one verification: the whole node (`scope.per: node`) or a single file (`scope.per: file`). The reviewer for an LLM aspect is an LLM that receives the aspect's content.md, any declared reference files, the subject files of the unit, and any per-unit companion files resolved by `companion.mjs` (when present), and checks every rule against the code. A deterministic aspect's `check.mjs` runs locally over the unit. Both kinds produce a verdict stored in the lock keyed by the pair.
...
A verdict is valid exactly while the inputs that produced it hash to the stored value. Any input change — an edited subject file, an edited `content.md` / `check.mjs`, an edited `scope`, or a change to which named tier the aspect uses — makes the pair **unverified**, and `yg check --approve` re-verifies it. For companion-bearing LLM aspects: editing `companion.mjs` invalidates EVERY pair of that aspect (like editing `content.md`); editing a file that the hook resolved as a companion invalidates only the pairs that read it — a billed re-verification. Plain LLM aspects without `companion.mjs` are unaffected by this mechanism. (A status flip is NOT an input, and neither is a tier's underlying config — only the tier NAME folds in, so swapping the model or provider behind a named tier never invalidates a verdict.) States are: **verified / unverified / refused**.
