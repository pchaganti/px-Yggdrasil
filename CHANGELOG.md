...
- **`unverified` groups now collapse by code only — one group regardless of how many aspects are unverified.** Previously, `yg check` produced one `unverified` block per `(code, aspectId)` pair, so editing a single aspect on a large repo generated N near-identical blocks with the same Why+Fix text. Now all `unverified` issues share one block; the aspect is shown per body line (`- <node>  aspect '<id>'`) instead of in the group header. `refused` (`aspect-violation-enforced`/`advisory`), `prompt-too-large`, and `aspect-companion-runtime-error` are unaffected — their reviewer reason / remedy depends on the specific aspect. Exported `CODE_ONLY_GROUP_CODES` from `group-issues.ts` for use in tests and tooling.
- **Group body lines now show the `what` detail for non-aspect structural issues.** In the grouped check output, member body lines previously showed only `- <node>` (or `- <node>  aspect '<id>'` for unverified). Structural and graph-validation errors — `when-predicate-invalid`, `log-entry-missing`, `mapping-path-missing`, `flow-node-broken`, `aspect-undefined`, `port-undefined`, and similar — carry a specific diagnostic in their `what` field that distinguishes each instance. These now surface on the body line (`- <node>  <what summary>`) so the agent can see the specific reason without running a separate command.
