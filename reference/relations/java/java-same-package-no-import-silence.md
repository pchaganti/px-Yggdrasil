...
`Sibling` declared in sibling files of the same package — no import line, so the
import-only extractor emits nothing. A same-node reference is benign (intra-node, not
a relation); a cross-node split package is a REAL tolerated false-NEGATIVE. Adding
bare-simple-name resolution to catch it would reintroduce the §6.5.5 precedence trap
and is FORBIDDEN.
