...
describe('4.3.0 → 4.4.0 migration end-to-end', () => {
  let repo: string;

  beforeEach(() => {
    repo = mkdtempSync(join(tmpdir(), 'mig43-44-'));
    mkdirSync(join(repo, '.yggdrasil', 'model', 'foo'), { recursive: true });
    writeFileSync(join(repo, '.yggdrasil', 'yg-config.yaml'), 'version: "4.3.0"\nparallel: 1\n');
    writeFileSync(
  ...
  it('runs to-4.4.0, writes version, then validator emits type-without-when-with-mapping', async () => {
    ...
    expect(config).toMatch(/version: "4\.4\.0"/);
  ...
  it('refuse-load triggers when config bumped above 4.4.0', async () => {
    writeFileSync(join(repo, '.yggdrasil', 'yg-config.yaml'), 'version: "4.5.0"\n');
