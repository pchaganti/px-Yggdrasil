...
- **ESLint boundary config now mirrors the full layer architecture (B2).** Added the `structure-adapter` element (`structure/*`) and the `command-support` element/rules to `eslint.config.js`, corrected the element patterns to be repo-relative, and made the per-layer allow-lists match the architecture (engine→structure-adapter/ast-adapter, structure-adapter→utility/ast-adapter/model, command-support→engine/formatter/…). The config now documents the intended layering accurately; enforcement activation is tracked separately (see Deferred — B1).
...
- **Activating ESLint cross-layer boundary enforcement (B1).** The `boundaries/dependencies` rules now accurately mirror the architecture (the `structure-adapter` and `command-support` elements were added, patterns corrected to be repo-relative), but they do not yet block: under eslint-plugin-boundaries v6 (brought in by the latest-deps bump) + flat config, the TypeScript `.js`→`.ts` import resolution does not engage, so cross-file import targets resolve to nothing and the rules silently pass. The standalone resolver is correct; the v6 wiring is not. The corrected config + the `eslint-import-resolver-typescript` devDep are in place — only the resolver integration (or a pin to a v5 line, or a path-only custom rule) remains. The relocations that make the layering legal (B4, B6) and the new `mapping-escapes-repo` validator (B3) and single structural-code source (B7) all landed. Tracked in `.temp/dogfood-report.md`.
