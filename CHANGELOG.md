...
- **Documentation for relation conformance across the rules / knowledge / user-docs surface.** The agent rules primer (`rules.ts`, regenerating `agent-rules.md`) now describes the built-in relation-conformance check — always-error, not an aspect, not status-governed, not suppressible, one-directional, mapped-target-only — and how to fix a refusal (declare the relation or remove the dependency). The `ports-and-relations`, `verification-and-lock` (the lock format and that relation conformance is computed live, not cached), `cli-reference` (the `relation-undeclared-dependency` issue code), and `suppress-syntax` (the check is not suppressible) knowledge topics gained matching sections, and the user docs (`core-concepts`, `cli-reference`) describe the check.
...
- **Relation-conformance documentation consistency pass.** The adopter-facing `core-concepts.md` told first-time adopters to run plain `yg check` to discover undeclared dependencies, but plain `yg check` is parse-free and only re-validates recorded verdicts — the detecting run is always `yg check --approve`; the instruction now says so (and notes the distinction). The `yg-config.yaml` schema comment for `parallel` no longer calls it the "batch approve" concurrency limit (there is no batch approve under the verdict lock) — it is the `yg check --approve` fill concurrency. The `yg-node.yaml` schema's `relations:` field gains a note that declared relations are load-bearing — enforced against real code dependencies by the built-in `relation-undeclared-dependency` check — so an agent reading the schema understands why the field matters.
