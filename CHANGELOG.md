...
- End-to-end test coverage for the full approve lifecycle. A new fixture repo (`tests/fixtures/e2e-lifecycle`) with deterministic (`check.mjs`) aspects at every status level plus one LLM aspect, and a hermetic E2E suite (`tests/e2e/cli-deterministic-lifecycle.test.ts`) that spawns the real `dist/bin.js` and asserts: fresh approve → baseline written, source-drift refusal on an enforced violation, fix → re-approve, advisory violation rendered as a non-blocking warning, draft aspect skipped, `yg-suppress` waiver, aspect-cascade batch approval via `--aspect`, and the command guards (`--node`+`--aspect` mutual exclusion, `--aspect --dry-run` rejection, node-not-found, `tree --depth` validation), plus the reviewer-unreachable fallback (deterministically forced via a dead endpoint). A companion `cli-ollama-reviewer.external.test.ts` (run via `npm run test:external`, excluded from CI) proves the LLM reviewer path works end-to-end against a real Ollama endpoint.

### Fixed

- Documentation–implementation consistency pass (knowledge topics + schemas): removed retired "AST/structure aspect" and "structure runner" vocabulary left over from the reviewer-type collapse (`writing-llm-aspects`, `drift-and-cascade`, `schemas/yg-aspect.yaml`); documented the two previously-undocumented blocking port error codes (`port-undefined`, `port-missing-aspect`) in `ports-and-relations`; documented the `deterministic-touched:<id>` per-node drift key alongside `tier-identity:` in `drift-and-cascade`; completed the `cli-reference` command summary (was missing `owner`/`deterministic-test`/`type-suggest`/`init`); corrected `conditional-aspects` to attribute `has_mapping` only to the `node` `when`-atom; and fixed a deterministic-aspect cookbook comment that mischaracterized when `ctx.graph.node` throws.
...
- `yg approve --aspect <id>` now includes nodes that drifted only because a cross-node file read by one of the aspect's graph-aware `deterministic` checks changed. `filterAspectCascadeNodes` reads each candidate node's baseline and attributes such cross-node paths to the aspect (via `deterministicTouchedFiles`), mirroring `selectDriftedAspects`; previously those nodes were silently excluded from the batch, forcing a `--node` fallback.
...
- A node no longer triggers a full LLM re-review when it drifts only because a cross-node file read by one of its graph-aware `deterministic` aspects changed. `selectDriftedAspects` now attributes such a change (tracked on the `deterministic-touched` layer as the related node's actual path) to the owning deterministic aspect via the node baseline's `deterministicTouchedFiles`, so only that local, zero-cost check re-runs and every LLM verdict is carried forward. Previously the raw cross-node path was un-attributable, forcing a node-global re-run of every effective LLM aspect — costly and non-deterministic (re-reviews flaked and had to be retried). `aspectDependencyKeys` gained an optional `deterministicTouchedFiles` argument threaded only through `selectDriftedAspects`; `filterAspectCascadeNodes` (the `--aspect` selector) is unchanged. Cascade messages for the synthetic `deterministic-touched:<id>` and `tier-identity:<id>` causes now name the owning aspect ("the set of files read by deterministic aspect '<id>'" / "the resolved reviewer tier for aspect '<id>'") instead of "declared by unknown aspect".
...
- **Internal rename (no behavior change):** the persisted-state tokens that still referenced the retired `structure`/`ast` reviewer kinds are renamed to the unified `deterministic` vocabulary — `structureTouchedFiles` → `deterministicTouchedFiles`, the `structureTouchedKey`/`structure-touched:<id>` synthetic drift key → `deterministicTouchedKey`/`deterministic-touched:<id>`, the `TrackedFileLayer` value `'structure-touched'` → `'deterministic-touched'`, and the `AspectVerdict.errorSource` value `'astRuntime'` → `'checkRuntime'`. These never shipped in a released version (5.0.0-only), so no migration is needed for 4.x adopters; in-repo baselines were migrated in place.
