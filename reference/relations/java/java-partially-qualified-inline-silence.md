...
import) is a usage site with no import line → no hint → silence. Reading the
partially-qualified chain as deeper packages, or binding `Outer` by bare simple name,
would reintroduce the §6.5.5 precedence trap and is FORBIDDEN.
