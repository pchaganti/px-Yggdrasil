...
## [2026-05-28T13:14:02.753Z]
Added check-aspect-status.test.ts covering the new yg check rendering by aspect status: aspect-newly-active (advisory and enforced both produce errors on missing baseline), aspect-violation-enforced and aspect-violation-advisory branches, draft silence, approved silence, legacy baseline tolerance (no aspectVerdicts field does not emit aspect-newly-active), and suggestedNext priority where an enforced error wins over a co-emitted advisory warning.
## [2026-05-28T13:47:03.403Z]
Extended approve-aspect-status tests with the --node Y all-draft scenarios analogous to Task 13's --aspect coverage. Two cases: (a) when every effective aspect on the target node is draft, the CLI emits the 'Reviewer skipped' message and no reviewer call occurs; (b) dry-run still prints prompts for ALL effective aspects regardless of status, but each prompt is tagged with its effective status and draft aspects carry the 'real approve would skip' annotation. The dry-run case drives the newly-extracted runDryRunForNode helper directly to capture stdout without spawning Commander.
## [2026-05-28T19:04:04.169Z]
Aspect-status final review test gap closure: added 11 unit cases for applyAspectVerdictsToResult merge semantics and reviewerAborted detection. Covers filtered vs full-node merge, reviewer-aborted preservation of prior verdicts (tier-resolution failure scenario), all-draft node writing empty {}, and the no-pendingDriftState no-op.
