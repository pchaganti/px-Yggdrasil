...
runtime behavior. It is generated by \`yg init\`. \`yg init --upgrade\` migrates
the graph (config, aspects, drift-state baselines) to the current version and
refreshes rules, schemas, and platform files.
...
Migrates the graph to the current version: applies registered migrations to
\`yg-config.yaml\`, all \`yg-aspect.yaml\` files, and drift-state baselines
(losslessly re-keyed to the typed format), then refreshes rules, schemas, and
platform files. The legacy single-section reviewer format (flat provider keys +
\`reviewer.active\`) is migrated to \`reviewer.tiers\` automatically. Run from
