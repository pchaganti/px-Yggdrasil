...
describe.skipIf(!distExists)('structure aspect lifecycle', () => {
  ...
  it('create → approve → edit → drift → re-approve → no drift', () => {
    ...
    // 3. Read baseline, assert structureTouchedFiles populated for touches-a
    const baseline = readBaseline(root, 'N');
    const stf = (baseline.structureTouchedFiles as Record<string, Record<string, string>> | undefined);
