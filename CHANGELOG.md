...
- **`yg relations --suggest` — read-only relation triage.** A new command detects every undeclared cross-node dependency the relation pass would flag and prints, per node, the offending `<file>:<line> → <target>` edges plus the exact `relations:` stanza to paste into that node's `yg-node.yaml` (with the allowed relation type computed from the architecture allow-list, or a dead-end note pointing at `yg-architecture.yaml` when no relation type connects the two node types). It is purely read-only: it never runs the LLM reviewer, never touches the lock, and always exits 0 — a fast way to see what to declare before running `yg check --approve`.
...
- **Documentation for relation conformance across the rules / knowledge / user-docs surface.** The agent rules primer (`rules.ts`, regenerating `agent-rules.md`) now describes the built-in relation-conformance check — always-error, not an aspect, not status-governed, not suppressible, one-directional, mapped-target-only — and how to fix a refusal (declare the relation, remove the dependency, or `yg relations --suggest`). The `ports-and-relations`, `verification-and-lock` (lock format v2, the `relation_verdicts` section, parse-free re-validation, and the v1→v2 migration), `cli-reference` (`yg relations --suggest` and the `relation-undeclared-dependency` issue code), and `suppress-syntax` (the check is not suppressible) knowledge topics gained matching sections, and the user docs (`core-concepts`, `cli-reference`) describe the check and the `yg relations --suggest` migration aid for adopters.
