...
- **Reviewer tiers** — `yg-config.yaml` now uses `reviewer.tiers.<name>` (named tier blocks). Each tier declares `provider`, `consensus`, and `config`. Aspects target a tier via `reviewer: { type: llm, tier: <name> }`; aspects without `tier:` use the first defined tier. Supported providers: `ollama`, `anthropic`, `openai`, `google`, `openai-compatible`, `claude-code`, `codex`, `gemini-cli`.
...
- Migration `to-5.0.0`: `transformConfigReviewer` converts v4 flat-provider config to `reviewer.tiers.standard`; `transformAspectReviewer` converts `reviewer: <string>` to `reviewer: { type: ... }`. Registered in `migrations/index.ts`.
...
- `yg-config.yaml` reviewer format moved from v4 (flat provider keys under `reviewer:`) to v5 (named tiers under `reviewer.tiers`). The v4 format now throws `config-reviewer-legacy-format` at parse time — run `yg init --upgrade` to migrate.
- `yg-aspect.yaml` reviewer field: string shorthand (`reviewer: llm`) is rejected with `aspect-reviewer-legacy-string`. Required format: `reviewer: { type: llm | ast }` with optional `tier:`.
